...
from modeling import BertForQuestionAnswering, BertConfig, WEIGHTS_NAME, \
    CONFIG_NAME
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if is_training and not example.is_impossible:
                ...
                # annotation we throw it out, since there is nothing to
                # predict.
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        null_start_logit = \
            0  # the start logit at the slice with min null score
        ...
        if version_2_with_negative:
            ...
            # In very rare edge cases we could only have single null
            # prediction. So we just create a nonce prediction in this case
            # to avoid failure.
            if len(nbest) == 1:
                nbest.insert(0,
        ...
        if not version_2_with_negative:
            ...
            # predict "" iff the null score - the score of best
            # non-null > threshold
    ...
    with chio.open(output_prediction_file, "w") as writer:
        writer.write(json.dumps(all_predictions, indent=4) + "\n")

    with chio.open(output_nbest_file, "w") as writer:
        writer.write(json.dumps(all_nbest_json, indent=4) + "\n")

    if version_2_with_negative:
        with chio.open(output_null_log_odds_file, "w") as writer:
...
def main():
    ...
    parser.add_argument("--output_dir", default=None, type=str, required=True,
                        help="The output directory where the model "
                             "checkpoints and predictions will be written.")
    ...
    # Other parameters
    parser.add_argument("--squad_file", default=None,
                        type=str, help="SQuAD zip file")
    ...
    parser.add_argument("--max_query_length", default=64, type=int,
                        help="The maximum number of tokens for the question. "
                             "Questions longer than this will be truncated "
                             "to this length.")
    ...
    parser.add_argument("--verbose_logging", action='store_true',
                        help="If true, all of the warnings related to data "
                             ...
                             "A number of warnings are expected for a "
                             "normal SQuAD evaluation.")
    ...
    parser.add_argument('--null_score_diff_threshold',
                        ...
                        help="If null_score - best_non_null is greater than "
                             "the threshold predict null.")
    parser.add_argument('--vocab_file',
                        type=str, default=None, required=True,
                        help="The zip file containing vocabulary mapping file")
    parser.add_argument('--vocab_path',
                        type=str, default=None, required=True,
                        help="Vocabulary mapping/file BERT was pretrainined"
                             "on")
    ...
    if args.do_predict:
        if not args.predict_file:
            raise ValueError(
                "If `do_predict` is True, then `predict_file` "
                "must be specified.")

    with chio.open_as_container(args.vocab_file) as container, \
            container.open(args.vocab_path, "r", encoding="utf8") as reader:
        # for bert large
        tokenizer = BertTokenizer(
            reader, do_lower_case=args.do_lower_case, max_len=512)

    train_examples = None
    num_train_optimization_steps = None
    if args.do_train:
        with chio.open_as_container(args.squad_file) as container, \
                container.open(args.train_file) as file:
            train_examples = read_squad_examples(
                input_file=file, is_training=True,
    ...
    with chio.open(args.init_checkpoint) as f:
        ...
        if args.fp16:
            try:
                ...
                raise ImportError(
                    ...
                    "https://www.github.com/nvidia/apex to use distributed "
                    "and fp16 training.")
            ...
            if args.loss_scale == 0:
                ...
                model, optimizer = amp.initialize(
                    model, optimizer, opt_level="O2",
                    keep_batchnorm_fp32=False, loss_scale=args.loss_scale)
            if not args.old and args.do_train:
                scheduler = LinearWarmUpScheduler(
                    optimizer, warmup=args.warmup_proportion,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            train_dataloader = tqdm(train_dataloader, desc="Iteration") if (
    ...
    if args.do_train and (args.local_rank == -1 or
        ...
        with chio.open(output_model_file, "wb") as file:
            torch.save(model_to_save.state_dict(), file)

        print("Save config")
        output_config_file = os.path.join(args.output_dir, CONFIG_NAME)
        with chio.open(output_config_file, "w") as f:
    ...
    if args.do_predict and (args.local_rank == -1 or
                            torch.distributed.get_rank() == 0):
        if not args.do_train and args.fp16:
            model.half()

        with chio.open_as_container(args.squad_file) as container, \
                container.open(args.predict_file) as f:
            eval_examples = read_squad_examples(
                input_file=f, is_training=False,
                version_2_with_negative=args.version_2_with_negative)
        ...
        eval_dataloader = tqdm(eval_dataloader, desc="Evaluating") if (
            args.local_rank == -1 or torch.distributed.get_rank() == 0
