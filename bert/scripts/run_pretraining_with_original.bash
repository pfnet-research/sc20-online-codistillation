...
burnin_seq=$(awk "BEGIN{print int($total_seq*0.4)}")
burnin_seq_phase2=$(awk "BEGIN{print int($total_seq*0.04)}")
base_learning_rate=0.0001
base_warmup=0.01
ratio=$(awk "BEGIN{print $train_batch_size / 256.0}")
steps=$(awk "BEGIN{print int(${total_seq}/$train_batch_size)}")
burnin_steps=$(awk "BEGIN{print int(${burnin_seq}/$train_batch_size)}")
burnin_steps_phase2=$(awk "BEGIN{print int(${burnin_seq_phase2}/$train_batch_size)}")
...
CMD+=" --optimizer=adam"
CMD+=" --seed=$seed"
CMD+=" --online_distillation original"
CMD+=" --burnin_steps ${burnin_steps}"
...
CMD+=" --seed=$seed"
CMD+=" --online_distillation original"
CMD+=" --burnin_steps $(( ${train_steps_phase1} + ${burnin_steps_phase2} ))"
CMD+=" --optimizer=adam"
