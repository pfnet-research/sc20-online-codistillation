...
# Known issues
# - dloss is added to loss before being registered to the stats in overlap (economical) and equalized (synchornized)
#   distillations but not in original distillation.
# - `dloss_stats` is not correctly cleared in `Trainer.report`.
# In our experiments, these issues were properly addressed at the time of log analysis.
...
class Trainer(object):
    def __init__(self, options=None):
        self.distribute_initialized = False
        if options is None:
    ...
    def setup_distributed(self):
        if self.distribute_initialized:
            return
        self.distribute_initialized = True
        ...
        if self.options.distillation:
            ...
            for i in range(2):
                team_groups.append(dist.new_group(ranks=list(range(i * self.team_size,
                                                                   (i + 1) * self.team_size))))
    ...
    def setup_model(self):
        ...
        if self.options.distillation:
            self.another_model = torchvision.models.resnet50(pretrained=False)
    ...
    def setup_optimizer(self):
        if self.options.lars_eta is not None:
            param_groups = [
                {'params': [v for n, v in self.model.named_parameters() if 'bn' in n or 'bias' in n], 'weight_decay': 0, 'lars_eta': 0.0 },
                {'params': [v for n, v in self.model.named_parameters() if 'bn' not in n and 'bias' not in n], 'weight_decay': self.options.weight_decay, 'lars_eta': self.options.lars_eta },
            ]
            optimizer = LARSSGD(param_groups,
    ...
    def exchange_output(self, output):
        ...
        with torch.no_grad():
            another_output = another_output - output.detach()
    ...
    def train_loop_distillation_overlap(self):
        ...
        while True:
            ...
            while True:
                try:
                    next_images, next_target = next(iterator)
                    next_images = next_images.cuda(non_blocking=True).clone()
                    next_target = next_target.cuda(non_blocking=True).clone()
                    break
    ...
    def train(self):
        ...
        if self.options.label_smoothing:
            self.criterion = LabelSmoothing(self.options.label_smoothing).cuda()
        else:
        ...
        if self.team_rank == 0 and self.options.out is not None:
            if self.options.distillation:
...
def main():
    ...
    if '@' not in argv:
        ...
        if len(last_run) >= 1:
            runs.append(last_run)
        global_opts = opt.parse_command_line(runs[0])
        trainer = None
        for run in runs[1:]:
            print(run)
            opts = opt.parse_command_line(run)
            opts.train_dir = global_opts.train_dir
            opts.val_dir = global_opts.val_dir
            if global_opts.local_rank is not None:
                opts.local_rank = global_opts.local_rank
            if trainer is None:
                trainer = Trainer(opts)
            else:
