...
class LearningRateScheduler(object):
    ...
    Args:
        func: a schedule function. It should take 4 arguments epoch, epoch_detail, iteration, and observation.
    """
...
def with_legw(func, warmup_epochs, target_lr):
    def f(func, warmup_epochs, epoch, epoch_detail, iteration):
        if epoch_detail < warmup_epochs:
            return target_lr * epoch_detail / warmup_epochs
        else:
            return func(epoch, epoch_detail, iteration)
    return partial(f, func, warmup_epochs)


def legw_roller_coaster(warmup_epochs, warmup_max, max_epoch):
    def f(epoch, epoch_detail, iteration):
        if epoch_detail < warmup_epochs:
            return warmup_max * epoch_detail / warmup_epochs
        else:
            return max(1e-6, warmup_max * (1.0 - float(epoch_detail) / max_epoch) / ((1.0 - float(warmup_epochs) / max_epoch)))
    return f
...
def get_learning_rate_scheduler(options, epoch, effective_batchsize, initial_wd=None):
    ...
    if options.legw or options.legw_roller_coaster:
        initial_lr = options.initial_lr * math.sqrt(effective_batchsize / 256.0)
    else:
    ...
    if options.legw_roller_coaster:
        schedule_func = legw_roller_coaster(options.warmup_epoch * (effective_batchsize / 256.0), initial_lr, epoch)
    elif options.legw:
        schedule_func = with_legw(schedule_func, options.warmup_epoch * (effective_batchsize / 256.0), initial_lr)
    elif options.warmup_epoch is not None:
